...
SOURCE = "https://m3u.cl/lista/total.m3u"
TIMEOUT = 10
MAX_CANDIDATES = 80
...
def infer_group(text):
    x = text.casefold()
    mapping = [
        (("alger", "algérie", "algerie"), "Algérie"),
        (("maroc", "morocco", "medi1"), "Maroc"),
        (("tunisie", "tunisia"), "Tunisie"),
        (("egypt", "egypte", "égypte"), "Égypte"),
        (("qatar",), "Qatar"),
        ...
        (("bahrain",), "Bahreïn"),
        (("oman",), "Oman"),
        (("mauritania", "mauritanie"), "Mauritanie"),
    ]
    for keys, group in mapping:
        if any(k in x for k in keys):
            return group
    if any(k in x for k in ("france", "français", "francais", "french", "tv5", "bfm", "arte")):
        return "France"
...
def probe(url):
    try:
        ...
        head = data.lstrip()
        low = url.casefold()
        if ".m3u8" in low or "playlist" in low or "manifest" in low or "master" in low:
...
def fetch_source():
    ...
    with urlopen(req, timeout=15, context=ctx) as r:
        return r.read(5_000_000).decode("utf-8", "ignore")
    ...
    while i < len(lines):
        if lines[i].startswith("#EXTINF:"):
            ...
            while j < len(lines) and not lines[j].startswith("#EXTINF:"):
                ...
                j += 1
            if url:
                out.append({"id": attrs.get("tvg-id", ""), "name": name, "logo": attrs.get("tvg-logo", ""), "group": attrs.get("group-title", ""), "url": url})
        i += 1
    return out

try:
    raw = fetch_source()
    source_entries = parse_m3u(raw)
except Exception as exc:
    print("M3U.CL source fetch failed:", type(exc).__name__)
    source_entries = []
...
for e in source_entries:
    ...
    if not any(k in hay for k in RELEVANT):
        continue
    if any(k in hay for k in PREMIUM):
        continue
    try:
        p = urlsplit(e["url"])
    except Exception:
    ...
    if u in seen:
        continue
    low = u.casefold()
    if not any(x in low for x in (".m3u8", "playlist", "manifest", "master", ".mpd")):
        continue
    seen.add(u)
    e["url"] = u
    e["country"] = infer_group(hay)
    candidates.append(e)
...
while i < len(lines):
    ...
    if line.startswith("#EXTINF:"):
        ...
        continue
    if not line.startswith("# =====") and line.strip() and not line.startswith("# M3U.CL vérifié:"):
        header.append(line)
    i += 1
...
with ThreadPoolExecutor(max_workers=16) as ex:
    ...
    for fut in as_completed(futs):
        e = futs[fut]; ok, status = fut.result(); results[e["url"]] = ok
        print(("PASS" if ok else "FAIL"), status, e["name"], e["url"])
...
for e in candidates:
    ...
    if matches:
        group, primary = matches[0]
        current_urls = {stream_url(b) for _, b in matches if stream_url(b)}
        ...
        ext = f'#EXTINF:-1 tvg-id="{tid}" tvg-logo="{logo}" group-title="{group}",{entry_name(primary[0])} (Secours {num})'
        block = [ext, u]; groups.setdefault(group, []).append(block); name_index.setdefault(nn, []).append((group, block)); backups += 1
    else:
        group = e["country"]; tid = e.get("id", "") or "M3UCL"; logo = e.get("logo", "")
        ext = f'#EXTINF:-1 tvg-id="{tid}" tvg-logo="{logo}" group-title="{group}",{e["name"]}'
        block = [ext, u]; groups.setdefault(group, []).append(block); name_index.setdefault(nn, []).append((group, block)); new_channels += 1
